...
struct OpFunction {
    ...
    func eval(_ args: [NSNumber?]) -> NSNumber {
        if args.count < self.minArgs || (self.maxArgs != -1 && args.count > self.maxArgs) {
            return 0
        }
        return self.evalFunc(args.compactMap { $0! })
    }
}
...
class BWSkipStepNavigationRule: ORKSkipStepNavigationRule {
    ...
    lazy var operatorMap: [String: OpFunction] = {
        ...
        opDict["=="] = OpFunction(minArgs: 2, maxArgs: 2) { args in
            (args[0].doubleValue == args[1].doubleValue) ? 1 : 0
        }
        opDict["<"] = OpFunction(minArgs: 2, maxArgs: 2) { args in
            (args[0].doubleValue < args[1].doubleValue) ? 1 : 0
        }
    ...
    func evaluateLogic(_ value: AnyObject, taskResult: ORKTaskResult) -> NSNumber? {
        ...
        case let questionId as NSString:
            ...
            if let questionIdStr = questionId as? String {
                ...
                if let stepResult = stepResult {
                    return self.valueForStepResult(stepResult)
                } else {
                    return nil
                }
            }
        ...
        case let dict as [String: AnyObject]:
            ...
            return self.evaluateLogic(OpTuple(op: "and", values: ar as AnyObject), taskResult: taskResult)
        case let opTuple as OpTuple:
            ...
            if let values = opTuple.values as? [AnyObject] {
                for v in values {
                    opValues.append(self.evaluateLogic(v, taskResult: taskResult))
                }
            } else {
                opValues.append(self.evaluateLogic(opTuple.values, taskResult: taskResult))
            }
